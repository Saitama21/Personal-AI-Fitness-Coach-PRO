...
function localCoachReply(message, context = {}) {
  ...
  if (/увелич|вес|прогресс/.test(text)) {
    return {
      provider: "local",
      text: "FORMA использует double progression: сначала доведи все рабочие подходы до верхней границы заданного диапазона при стабильной технике и RPE не выше 8. Только после этого повышается нагрузка небольшим шагом. Если часть подходов остаётся внутри диапазона — вес сохраняется, а следующая цель состоит в добавлении повторений."
    };
  }
